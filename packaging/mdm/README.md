...
| [Intune playbook](../docs/mdm/intune.md) · [中文](../docs/mdm/intune-zh.md) | macOS + Linux + Intune |
...
| Intune | [`examples/intune/`](examples/intune/) — plist, sentinel, enroll, Linux systemd |
...
4. Issue one-time enroll token; MDM injects `FINSAFE_ENROLL_TOKEN` + device id; remove token after `/etc/finsafe/enrolled.json` exists.
5. Apps invoke `finsafe run -- <program>` (no local `--policy`).

## Binary digest attestation

Heartbeats include SHA-256 of binaries at `/usr/local/bin/finsafe` and `finsafe-agent`. Pin expected digests in sentinel `expected_binary_digests` when signing (extend `finsafe-bundlectl` workflow as needed).
